...
# Q5 Plot a chi-sq distribution with 5 degrees of freedom.

```
# Lab 1 Written answer question
The following is the code for Q1
```{r}
x <- seq(-4, 10, 0.1)
plot(x, dnorm(x, mean = 2, sd = 0.4), type = "l",
     ylim = c(0, 5), ylab = "", lwd = 2, col = "red")

```

The following is the code for Q2
```{r}
qnorm(0.85, mean=2, sd=0.4)
```

The following is the code for Q3
```{r}
a <- pnorm(1, 2, 0.4)
b <- pnorm(2, 2, 0.4)
prob.between.1.and.2 <- b - a
print(prob.between.1.and.2)
```

The following is the code for Q4
```{r}
normal.a <- rnorm( n=1000, mean=2, sd=0.4 ) 
normal.b <- rnorm( n=1000 , mean=2, sd=0.4 )  
normal.c <- rnorm( n=1000 , mean=2, sd=0.4 )  
normal.d <- rnorm(n =1000 , mean=2, sd=0.4)
normal.e <- rnorm (n = 1000 , mean=2, sd=0.4)
chi.sq.5 <- (normal.a)^2 + (normal.b)^2 + (normal.c)^2 + (normal.d)^2 + (normal.e)^2
hist(chi.sq.5)
```


```{r}
scaled.chi.sq.5 <- chi.sq.5 / 5
normal.f <- rnorm(n = 1000 , mean=2, sd=0.4)
t.5 <- normal.f / sqrt( scaled.chi.sq.5 )  
hist (t.5)
```
